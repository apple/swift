...
Parser::parseExprPostfixSuffix(ParserResult<Expr> Result, bool isExprBasic,
  ...
  while (1) {
    ...
    if (Tok.isFollowingLSquare()) {
      auto argList = parseArgumentList(tok::l_square, tok::r_square,
                                       isExprBasic);

      Result = makeParserResult(ParserStatus(argList) | Result,
                                SubscriptExpr::create(Context, Result.get(),
                                                      argList.get(),
                                                      ConcreteDeclRef(),
                                                      /*implicit=*/false));
    ...
    if (Tok.is(tok::l_brace) && isValidTrailingClosure(isExprBasic, *this)) {
      ...
      auto *argList =
          ArgumentList::create(Context, SourceLoc(), trailingClosures,
                               SourceLoc(), /*firstTrailingClosureAt*/ 0,
                               /*implicit*/ false);
      Result = makeParserResult(
    ...
    case Lexer::StringSegment::Literal: {
      ...
      auto AppendLiteralRef =
        new (Context) UnresolvedDotExpr(InterpolationVarRef,
                                        ...
                                        /*nameloc=*/DeclNameLoc(), 
                                        /*Implicit=*/true);
      auto *ArgList = ArgumentList::forImplicitUnlabelled(Context, {Literal});
...
ParserResult<Expr> Parser::parseTupleOrParenExpr(tok leftTok, tok rightTok) {
  ...
  // FIXME: Change TupleExpr to store a single array of elements.
...
Parser::parseArgumentList(tok leftTok, tok rightTok, bool isExprBasic,
  ...
  // FIXME: Introduce new SyntaxKind for ArgumentList?
  auto status =
  ...
  // we're done.
  if (!allowTrailingClosure || Tok.isNot(tok::l_brace) ||
  ...
  auto numNormalArgs = args.size();

  // Parse the closure.
  status |=
      parseTrailingClosures(isExprBasic, SourceRange(leftLoc, rightLoc), args);
  Optional<unsigned> firstTrailingClosureAt;
  if (args.size() > numNormalArgs)
    firstTrailingClosureAt = numNormalArgs;

  auto *argList = ArgumentList::create(Context, leftLoc, args, rightLoc,
                                       firstTrailingClosureAt,
                                       /*implicit*/ false);
  return makeParserResult(status, argList);
}
...
Parser::parseTrailingClosures(bool isExprBasic, SourceRange calleeRange,
  ...
  closures.push_back(Argument::unlabelled(closure.get()));
...
ParserResult<Expr> Parser::parseExprPoundUnknown(SourceLoc LSquareLoc) {
  ...
  if (ArgList && !ArgList->empty() && ArgList->front().getLabelLoc().isValid())
...
Parser::parseExprCallSuffix(ParserResult<Expr> fn, bool isExprBasic) {
  ...
  if (peekToken().is(tok::code_complete) && CodeCompletion) {
    ...
    auto *argList = ArgumentList::create(
        Context, lParenLoc, {Argument::unlabelled(CCE)}, rParenLoc,
        /*trailingClosure*/ None, /*implicit*/ false);
