...
RValue SILGenFunction::emitApply(
  ...
  // the 'self' parameter.
  ManagedValue lifetimeExtendedSelfMV;
  if (hasUnownedInnerPointerResult(substFnType)) {
    ...
    case ParameterConvention::Direct_Owned:
      // The callee consumes self.  Copy self first--the returned inner pointer
      // will depend on the copy's lifetime.
      lifetimeExtendedSelfMV = selfMV.copy(*this, loc);
      break;
    case ParameterConvention::Direct_Guaranteed:
    case ParameterConvention::Direct_Unowned:
      // Self may be guaranteed only for the duration of the ArgumentScope.
      // Create a copy whose lifetime can be extended into the enclosing scope
      // so that the returned inner pointer can depend on the copy.
      lifetimeExtendedSelfMV = selfMV.copy(*this, loc);
      break;
  ...
  if (lifetimeExtendedSelfMV && lifetimeExtendedSelfMV.hasCleanup()) {
    argScope.popPreservingValue(lifetimeExtendedSelfMV);
  } else {
    argScope.pop();
  }
  ...
  auto addManagedDirectResult = [&](SILValue result,
    ...
    case ResultConvention::UnownedInnerPointer: {
      assert(hasUnownedInnerPointerResult(substFnType));
      assert(lifetimeExtendedSelfMV &&
       "did not save lifetime-extended self param");
      result = B.createMarkDependence(loc, rawDirectResult, lifetimeExtendedSelfMV.getValue());
      break;
    }
