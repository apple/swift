...
bool RValueTreatedAsLValueFailure::diagnoseAsError() {
  ...
  if (auto callExpr = dyn_cast<ApplyExpr>(diagExpr)) {
    ...
    if (auto *call = dyn_cast_or_null<ApplyExpr>(findParentExpr(inoutExpr))) {
      ...
      while (argIdx < args->size()) {
        if (args->getExpr(argIdx) == inoutExpr)
          break;
        argIdx++;
      }
...
bool AssignmentFailure::diagnoseAsError() {
  ...
  if (auto *AE = dyn_cast<ApplyExpr>(immutableExpr)) {
    ...
    if (isa<CallExpr>(AE) && AE->isImplicit() && args->isUnary()) {
      if (auto LE = dyn_cast<LiteralExpr>(
              args->getExpr(0)->getSemanticsProvidingExpr())) {
        emitDiagnosticAt(Loc, DeclDiagnostic, "literals are not mutable")
            .highlight(LE->getSourceRange());
        return true;
      }
  ...
  if (auto *SE = dyn_cast<SubscriptExpr>(expr)) {
    ...
    if (member) {
      ...
      if (!member->isDecl()) {
        ...
        auto *unaryArg = SE->getArgs()->getUnaryArgExpr();
        assert(unaryArg);
...
bool ContextualFailure::diagnoseConversionToNil() const {
  ...
  if (auto contextualTy =
    ...
    } else if (parentExpr->getArgs() || isa<KeyPathExpr>(parentExpr)) {
      Expr *fn = nullptr;
      if (auto *apply = dyn_cast<ApplyExpr>(parentExpr))
        fn = apply->getFn();
      if (auto *subscript = dyn_cast<SubscriptExpr>(parentExpr))
        fn = subscript->getBase();

      // If the 'nil' is in the fn, it's something like nil(5), which can't be
      // inferred without a contextual type.
      if (fn == getAsExpr(anchor)) {
        emitDiagnostic(diag::unresolved_nil_literal);
        return true;
      }

      // Otherwise, `nil` is passed as an argument to a parameter which doesn't
...
bool ContextualFailure::diagnoseConversionToBool() const {
  ...
  if (auto *parent = findParentExpr(anchor)) {
    if (auto *parentOpCall = dyn_cast<PrefixUnaryExpr>(parent)) {
      ...
      if (opRef && opRef->getDecl()->getBaseName() ==
...
bool ContextualFailure::tryIntegerCastFixIts(
  ...
  auto getInnerCastedExpr = [&](const Expr *expr) -> Expr * {
    ...
    if (!isa<ConstructorRefCallExpr>(CE->getFn()))
      return nullptr;
    return CE->getArgs()->getUnlabelledUnaryExpr();
  };
...
bool InvalidProjectedValueArgument::diagnoseAsError() {
  ...
  if (!param->hasAttachedPropertyWrapper()) {
    param->diagnose(diag::property_wrapper_param_no_wrapper, param->getName());
             ...
             param->getAttachedPropertyWrappers().front()->getArgs()) {
...
bool MissingMemberFailure::diagnoseForSubscriptMemberWithTupleBase() const {
  ...
  if (auto *argExpr = args->getUnaryArgExpr()) {
    auto *literal =
...
bool MissingArgumentsFailure::diagnoseSingleMissingArgument() const {
  ...
  auto firstTrailingClosure = args->getFirstTrailingClosureIndex();
...
bool MissingArgumentsFailure::diagnoseInvalidTupleDestructuring() const {
  ...
  auto *argExpr = args->getUnaryArgExpr();
...
bool ExtraneousArgumentsFailure::diagnoseSingleExtraArgument() const {
  ...
  auto loc = arguments->getExpr(index)->getLoc();
  if (index == arguments->size() - 1 && arguments->hasAnyTrailingClosures()) {
    emitDiagnosticAt(loc, diag::extra_trailing_closure_in_call)
        .highlight(arguments->back().getSourceRange());
  } else if (ContextualType->getNumParams() == 0) {
    auto *subExpr = arguments->getUnaryArgExpr();
